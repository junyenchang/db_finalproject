<?php
    ...
    if(mysqli_num_rows($result) >= 8){
        ...
        echo "<a href='afterLog/index_afterLog.html'>點此回到主頁面</a>";
        header('refresh:32; url = afterLog/index_afterLog.html');
    }
    if($row_playername == NULL){
        echo "請重新確認您的身分證字號是否輸入正確";
        echo "<a href='tennis.html'>未成功跳轉頁面請點擊此</a>";
        header('refresh:32; url = tennis.html');
    }

    else if($row_playername['username'] != $row_check['username']){
        echo "請重新確認您的身分證字號是否輸入正確";
        echo "<a href='tennis.html'>未成功跳轉頁面請點擊此</a>";
        header('refresh:32; url = gtennis.html');
    }
    else{
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            $playername = $row_playername['username'] ;
            $sql = "SELECT * FROM tennis WHERE user_id ='$user_id'";
            $result=mysqli_query($conn,$sql);
            if (mysqli_num_rows($result)==0 ){  //資料庫沒有相同的帳號
                do{$NUM = rand(1,8);
                    $check=" SELECT * FROM tennis WHERE NUM ='".$NUM."'" ;
                    }
                    while(mysqli_num_rows(mysqli_query($conn,$check))==1);
                $sql_fin = "INSERT INTO tennis ( NUM, user_id, playername, id) VALUES('$NUM','$user_id','$playername','$id')";
