...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    $check_account = "SELECT * FROM badminton WHERE id='$id' AND  user_id ='$user_id' ";

    if($row_playername == NULL){
        echo "請重新確認您的身分證字號是否輸入正確";
        echo "<a href='badminton_delete.html'>未成功跳轉頁面請點擊此</a>";
        header('refresh:32; url = badminton_delete.html');
    }
        ...
        echo "您並沒有報名此賽事<br>";
        echo "<a href= 'badminton.html'>請點擊此前往報名" ;
        return false;
    }
    else if(mysqli_num_rows(mysqli_query($conn,$check_account))==1){
            $sql="DELETE FROM  badminton WHERE user_id = '$user_id'";
