<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        $sql = "SELECT * FROM baseball WHERE user_id ='".$user_id."'";
        $result=mysqli_query($conn,$sql);
        if (mysqli_num_rows($result)==0 ){//資料庫沒有相同的帳號
            do{$NUM = rand(1,8);
                $check=" SELECT * FROM baseball WHERE NUM ='".$NUM."'" ;
                }
                while(mysqli_num_rows(mysqli_query($conn,$check))==1);  
            $sql="INSERT INTO baseball ( NUM, user_id, playername, id, teamname)
                VALUES('".$NUM."' ,'".$user_id."','".$playername."','".$id."','".$teamname."')";
            
            if(mysqli_query($conn, $sql)){
                    $sql_after = "SELECT teamname FROM baseball WHERE user_id = '$user_id'";
                    ...
                    echo "<a href='baseball_team.html'>點擊此處進行隊員報名</a>";
                    header("refresh:32;url=baseball_team.html");
                    exit;
                }else{
