...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    $check_account = "SELECT * FROM baseball WHERE id='$id' AND  user_id ='$user_id' ";
    if(mysqli_num_rows(mysqli_query($conn,$check_account))==0){
        echo "您並沒有報名此賽事<br>" ;
        return false;
    }
    if(mysqli_num_rows(mysqli_query($conn,$check_account))==1){
        $sql_team="DELETE FROM baseballteam WHERE teamname = '$teamname'";
        if(mysqli_query($conn,$sql_team)){
            $sql="DELETE FROM  baseball WHERE user_id = '$user_id'";
